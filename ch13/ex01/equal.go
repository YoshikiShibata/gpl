...
func equal(x, y reflect.Value, seen map[comparison]bool) bool {
	...
	case reflect.Float32, reflect.Float64:
		return equalFloats(x.Float(), y.Float()) // Exercise 13.1

	case reflect.Complex64, reflect.Complex128:
		return equalComplexes(x.Complex(), y.Complex()) // Exercise 13.1
	//!+
...
func equalFloats(x, y float64) bool {
	lowX := x - FloatDiff
	highX := x + FloatDiff
	return lowX <= y && y <= highX
}

func equalComplexes(x, y complex128) bool {
	return equalFloats(real(x), real(y)) &&
		equalFloats(imag(x), imag(y))
}
